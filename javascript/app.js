...
class Juego {
        ...
        if (numColor === this.secuencia[this.subNivel]){
            ...
            if(this.subNivel === this.nivel){
                ...
                if(this.nivel === (ULTIMO_NIVEL + 1))
                    ...
                    swal ('Balta Dice', `Pasaste el Nivel: ${this.nivel - 1}`, 'success')
    ...
    ganoElJuego () {
        swal ('Balta Dice', 'Felicitaciones, Ganaste el juego!', 'success')
        .then(this.inicializar);
    }

    perdioElJuego () {
        swal ('Balta Dice', 'Lo siento, Has perdido', 'error')
        .then(() =>{
            this.eliminarEventosBoton();
            this.inicializar();
        })
...
function empezarJuego(){
    swal('Balta Dice', 'El juego va a iniciar', 'info');
